...
const sections = [
  ["overview", "Обзор"], ["staff", "Сотрудники"],
  ["appeals", "Обращения"], ["audit", "Аудит"],
  ...
  ["support", "Экстренная помощь"], ["settings", "Настройки"],
] as const
...
function CreatePanel({
  ...
  return <Panel title={title} note={note}>{open ? children(() => setOpen(false)) : <Button type="button" onClick={() => setOpen(true)}>{action}</Button>}</Panel>
}

function CreateActions({ busy, close }: { busy: boolean; close: () => void }) {
  return <div className="flex gap-2 sm:col-span-2"><Button type="submit" disabled={busy}>{busy ? "Сохраняем…" : "Создать"}</Button><Button type="button" variant="outline" disabled={busy} onClick={close}>Отмена</Button></div>
}
...
export default function AdminWorkspacePage() {
  ...
  return <main className="staff-workspace"><StaffHeader title="Управление продуктом" role="Администратор" name={actor.display_name} onLogout={() => { void logout().finally(() => router.replace("/staff/login")) }} />
    <div className="mx-auto grid max-w-7xl gap-5 p-4 sm:p-6 lg:grid-cols-[250px_1fr]"><nav aria-label="Разделы администрирования" className="soft-scrollbar flex gap-1 overflow-x-auto rounded-2xl border border-slate-200/80 bg-white p-2 shadow-sm lg:sticky lg:top-24 lg:flex-col lg:self-start">{sections.map(([id, label]) => <button key={id} onClick={() => setSection(id)} className={`shrink-0 rounded-xl px-3 py-2.5 text-left text-sm font-medium ${section === id ? "bg-teal-700 text-white shadow-sm" : "text-slate-600 hover:bg-slate-100 hover:text-slate-950"}`}>{label}</button>)}</nav><div className="min-w-0 space-y-5">
      {failed ? <p className="rounded-xl bg-rose-50 p-3 text-sm text-rose-700">Часть данных не загрузилась.</p> : null}{message ? <p className="rounded-xl bg-teal-50 p-3 text-sm text-teal-800">{message}</p> : null}{error ? <p className="rounded-xl bg-rose-50 p-3 text-sm text-rose-700">{error}</p> : null}
...
function ChartPanel({ title, data, kind = "bar" }: { title: string; data: Array<{ key?: string; date?: string; count: number }>; kind?: "bar" | "line" }) {
  return <Panel title={title}>{data.length ? <div className="h-64"><ResponsiveContainer width="100%" height="100%">{kind === "line" ? <LineChart data={data}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="date" /><YAxis allowDecimals={false} /><Tooltip /><Line dataKey="count" stroke="#0f766e" /></LineChart> : <BarChart data={data}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="key" hide /><YAxis allowDecimals={false} /><Tooltip /><Bar dataKey="count" fill="#0f766e" /></BarChart>}</ResponsiveContainer></div> : <p className="text-sm text-slate-500">Нет данных за период.</p>}</Panel>
}
...
function AppealsView({ items, staff, request, run, busy }: { items: AdminAppealItem[]; staff: StaffItem[]; request: StaffRequest; run: Run; busy: boolean }) {
  ...
  return <Panel title="Зависшие обращения" note="Только метаданные. Содержимое, сообщения, заметки, вложения и контакты здесь недоступны.">{items.length ? <div className="space-y-3">{items.map((item) => <div key={item.id} className="rounded-xl border p-4"><div className="flex flex-wrap justify-between gap-3"><div><p className="font-medium">{item.category_name ?? "Без категории"} · {item.applicant_type}</p><p className="text-xs text-slate-500">{item.id} · {item.status} · {item.priority}</p><p className="text-sm">Специалист: {item.assigned_expert_display_name ?? "не назначен"}</p></div>{item.crisis_flag ? <span className="text-sm text-amber-700">Кризисный флаг</span> : null}</div><div className="mt-3 flex flex-wrap gap-2"><Button size="sm" variant="outline" disabled={busy} onClick={() => { const status = window.prompt("Статус: new, assigned, in_progress, needs_clarification или returned", item.status); const why = reason(); if (status && why) void run(() => adminApi.interveneAppeal(request, item.id, { status, reason: why })) }}>Статус</Button><Button size="sm" variant="outline" disabled={busy} onClick={() => { const priority = window.prompt("Приоритет: low, standard или urgent", item.priority); const why = reason(); if (priority && why) void run(() => adminApi.interveneAppeal(request, item.id, { priority, reason: why })) }}>Приоритет</Button><Button size="sm" variant="outline" disabled={busy} onClick={() => { const expert = window.prompt(`UUID специалиста (пусто — снять назначение):\n${experts.map((value) => `${value.display_name}: ${value.id}`).join("\n")}`, item.assigned_expert_id ?? ""); if (expert === null) return; const why = reason(); if (why) void run(() => adminApi.interveneAppeal(request, item.id, { assigned_expert_id: expert || null, status: expert ? "assigned" : "new", reason: why })) }}>Назначение</Button></div></div>)}</div> : <p className="text-sm text-slate-500">Обращений пока нет.</p>}</Panel>
}
...
function StaffView({ items, request, run, busy }: { items: StaffItem[]; request: StaffRequest; run: Run; busy: boolean }) {
  ...
  return <><CreatePanel action="Добавить сотрудника" title="Сотрудники" note="После создания временный пароль будет показан один раз.">{(close) => <form className="grid gap-3 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); const form = event.currentTarget; const f = new FormData(form); const expert = newRole === "expert"; let created: StaffCreateResult | null = null; void run(async () => { created = await adminApi.createStaff(request, { login: f.get("login"), email: f.get("email"), display_name: f.get("name"), role: newRole, ...(expert ? { public_specialist_label: f.get("label"), max_active_appeals: Number(f.get("capacity") || 10) } : {}) }); return created }, "Сотрудник создан.").then((saved) => { if (saved && created) { setCredentials(created); form.reset(); setNewRole("operator"); close() } }) }}><Field label="Логин" name="login" required /><Field label="Email" name="email" type="email" required /><Field label="Отображаемое имя" name="name" required /><div><Label htmlFor="role">Роль</Label><select id="role" name="role" value={newRole} onChange={(event) => setNewRole(event.target.value as typeof newRole)} className="h-10 w-full rounded-md border px-3"><option value="operator">Оператор</option><option value="expert">Эксперт</option><option value="admin">Администратор</option></select></div>{newRole === "expert" ? <><Field label="Специализация эксперта" name="label" required /><Field label="Вместимость" name="capacity" type="number" required /></> : null}<CreateActions busy={busy} close={() => { setNewRole("operator"); close() }} /></form>}</CreatePanel>
    {credentials ? <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/45 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="credentials-title"><div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl sm:p-8"><div className="mb-5 flex size-11 items-center justify-center rounded-full bg-teal-100 text-xl text-teal-800">✓</div><h2 id="credentials-title" className="text-2xl font-semibold tracking-tight">Сотрудник создан</h2><p className="mt-2 text-sm leading-6 text-slate-600">Сохраните данные сейчас — пароль больше не будет показан.</p><div className="my-6 space-y-3 rounded-2xl border border-slate-200 bg-slate-50 p-4"><p className="text-sm text-slate-500">Логин</p><p className="break-all font-mono text-lg font-semibold">{credentials.staff.login}</p><p className="pt-2 text-sm text-slate-500">Временный пароль</p><p className="break-all font-mono text-lg font-semibold">{credentials.temporary_password}</p></div><div className="grid gap-2 sm:grid-cols-2"><Button type="button" onClick={() => void copyCredentials()}>Копировать данные</Button><Button type="button" variant="outline" onClick={() => setCredentials(null)}>Готово</Button></div></div></div> : null}
