  ...
  return (
    <PublicShell>
      <section className="mx-auto max-w-2xl space-y-6 py-8 sm:py-12">
        ...
        {appeal ? <>
          <div className="surface-card overflow-hidden p-6 sm:p-9"><div className="-mx-9 -mt-9 mb-6 h-1.5 bg-teal-600" /><p className="text-xs font-semibold tracking-[0.14em] text-teal-700 uppercase">Текущий статус</p><h1 className="mt-2 text-3xl font-semibold tracking-tight">{appeal.status_text}</h1>{appeal.category ? <p className="mt-4 inline-flex rounded-full bg-slate-100 px-3 py-1.5 text-sm text-slate-700">Тема: {appeal.category.name}</p> : null}<p className="mt-3 text-xs text-slate-500">Обновлено {new Date(appeal.updated_at).toLocaleString("ru-RU")}</p>{appeal.rejection_reason ? <div className="mt-5 rounded-xl bg-slate-50 p-4 text-sm leading-6">{appeal.rejection_reason}</div> : null}</div>
          <div className="surface-card p-6"><h2 className="font-semibold">История статуса</h2><ol className="mt-5 space-y-5">{appeal.timeline.map((item, index) => <li key={`${item.status}-${item.occurred_at}`} className="relative flex gap-4"><span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-teal-100 text-sm font-semibold text-teal-800 ring-4 ring-white">{index + 1}</span><div><p className="font-medium">{item.text}</p><p className="mt-1 text-xs text-slate-500">{new Date(item.occurred_at).toLocaleString("ru-RU")}</p></div></li>)}</ol></div>
          {appeal.show_crisis_support ? <CrisisPanel resources={appeal.crisis_support_resources} /> : null}
          {messages.length || ["in_progress", "needs_clarification", "answer_ready"].includes(appeal.status) ? <section className="surface-card p-6"><div className="flex items-center justify-between gap-3"><h2 className="font-semibold">Диалог со специалистом</h2><span className="rounded-full bg-teal-50 px-2.5 py-1 text-xs text-teal-700">Конфиденциально</span></div><div className="my-5 space-y-3">{messages.map((item) => <div key={item.id} className={`rounded-2xl p-3.5 text-sm leading-6 ${item.author_type === "applicant" ? "ml-6 bg-teal-50 sm:ml-12" : "mr-6 bg-slate-100 sm:mr-12"}`}><p className="mb-1 text-xs font-medium text-slate-500">{item.author_label}</p><p className="whitespace-pre-wrap">{item.body}</p></div>)}</div>{["in_progress", "needs_clarification"].includes(appeal.status) ? <><Textarea value={message} onChange={(event) => setMessage(event.target.value)} maxLength={5000} placeholder="Напишите специалисту"/><Button className="mt-3 w-full sm:w-auto" disabled={busy || !message.trim()} onClick={() => void act(() => publicAppealsApi.sendMessage(message), "Сообщение отправлено.").then(() => setMessage(""))}>Отправить сообщение</Button></> : null}</section> : null}
